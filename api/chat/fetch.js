...
export default async function handler(req,res){
  ...
  const { data: rows } = await sb.from('chat_messages').select('user_id,text,created_at').eq('h3r7', memb.h3r7).order('created_at',{ascending:false}).limit(100);
  res.status(200).json({ h3r7: memb.h3r7, items: rows || [] });
}
