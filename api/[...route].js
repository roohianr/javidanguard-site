...
function getOp(url, path, base) {
  ...
  const parts = path.split('/'); // "auth/reg-start"
...
function handleHealth(req, res) {
  return res.status(200).json({
    ...
    rpName: process.env.RP_NAME || null,
    node: process.version,
  });
}
function handleK(req, res) {
  return res.status(200).json({ K, NOISE_B });
}

// ---------- MAP: agg + submit + admin ----------
async function handleAgg2(req, res, url) {
  // Members-only: require session
...
async function handleSubmit(req, res) {
  ...
  const { error: ins } = await client
    .from('submissions')
    .insert({ h3: cell, h3r7, h3r6, h3r5, bucket, hashed_device: hashedDevice });
...
async function handleAdminSeed(req, res) {
  ...
  const rows = Array.from({ length: N }, () => ({
    ...
    h3r5,
    bucket: b,
    hashed_device: randomBytes(16).toString('hex'),
  }));
  const client = sb();
...
async function handleRegStart(req, res) {
  try {
    ...
    if (!rpID) return res.status(500).json({ message: 'Missing env RP_ID' });

    const { handle = 'user' } = await getJSON(req);
    const userID = randomBytes(16).toString('base64url'); // REQUIRED
    ...
    return res.status(500).json({ message: 'reg-start failed', detail: e?.message || String(e) });
  }
}
...
async function handleRegFinish(req, res) {
  ...
  const body = await getJSON(req);
  const expectedChallenge = getCookie(req, 'wchal') || body.expectedChallenge; // fallback
  ...
  try {
    ...
    return res.status(400).json({ message: 'Registration error', detail: e?.message || String(e) });
  }
}
...
async function handleLoginFinish(req, res) {
  ...
  try {
    ...
    const { data: pk } = await client
      .from('passkeys')
      .select('user_id,counter,id')
      .eq('cred_id', credId)
      .maybeSingle();
    ...
    return res.status(400).json({ message: 'Login error', detail: e?.message || String(e) });
  }
}
...
async function handleMe(req, res) {
  ...
  const { data: memb } = await client
    .from('memberships')
    .select('cell_h3,bucket,locked_until,h3r7')
...
async function handleUpsertZone(req, res) {
  ...
  if (!cell_h3 || !h3.isValidCell?.(cell_h3))
    return res.status(400).json({ message: 'Valid cell_h3 required' });
  if (typeof bucket !== 'number' || bucket < 2 || bucket > 4)
    return res.status(400).json({ message: 'Group must be 5+' });
  ...
  const now = new Date();
  if (cur && new Date(cur.locked_until) > now) {
    return res.status(429).json({ message: 'Zone change locked', locked_until: cur.locked_until });
  }

  const locked_until = new Date(now.getTime() + 7 * 24 * 3600 * 1000).toISOString();
  const h3r7 = h3.cellToParent(cell_h3, 7),
    h3r6 = h3.cellToParent(cell_h3, 6),
    h3r5 = h3.cellToParent(cell_h3, 5);
...
async function handleInboxList(req, res) {
  ...
  const { data: msgs } = await client
    ...
    .or(`to_user.eq.${sess.user_id},to_user.is.null`)
...
async function handleChatPost(req, res) {
  ...
  if (!text || text.length < 1 || text.length > 500)
    return res.status(400).json({ message: 'Bad text' });
  const client = sb();
  const { data: memb } = await client
    .from('memberships')
    .select('h3r7')
  ...
  const { error } = await client
    .from('chat_messages')
    .insert({ user_id: sess.user_id, h3r7: memb.h3r7, text });
...
async function handleAnnCreate(req, res) {
  ...
  if (!cell_h3 || !h3.isValidCell?.(cell_h3))
    return res.status(400).json({ message: 'Valid cell required' });
  ...
  const { error } = await client
    .from('annotations')
    .insert({ author_id: sess.user_id, cell_h3, kind, title, details });
...
async function handleAnnVote(req, res) {
  ...
  const { error: vErr } = await client
    ...
    .upsert({ annotation_id: id, voter_id: sess.user_id, value });
  ...
  const { data: vs } = await client
    ...
    .select('value')
    .eq('annotation_id', id);
  const up = (vs || []).filter((v) => v.value === 1).length;
  const down = (vs || []).filter((v) => v.value === -1).length;
