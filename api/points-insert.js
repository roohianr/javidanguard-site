...
async function getJson(req){ if(req.body) return req.body; const chunks=[]; for await (const c of req) chunks.push(c);
...
async function userFromSid(req){
  ...
  const { data, error }=await db.from('sessions').select('user_id').eq('sid',sid)
    .gt('created_at', new Date(Date.now()-1000*60*60*24*60).toISOString()).single();
  if(error) return null; return data?.user_id||null;
}

export default async function handler(req,res){
  try{
    if(req.method!=='POST') return send(res,405,{ok:false,message:'Method not allowed'});
    const uid=await userFromSid(req); if(!uid) return send(res,401,{ok:false,message:'Login required'});
    const body=await getJson(req); const cell=body?.cell; const value=Number(body?.value);
