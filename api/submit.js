...
export default async function handler(req, res) {
  try {
    ...
    if (qErr) return res.status(500).json({ message: 'DB query error', detail: qErr.message || qErr });
    if (recent) return res.status(429).json({ message: 'You already submitted here recently.' });
