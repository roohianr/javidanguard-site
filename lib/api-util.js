...
export async function sessionUser(req) {
  ...
  if (!sid) return null;
  const { data, error } = await db.from('sessions').select('user_id, expires_at').eq('sid', sid).single();
...
export function subpath(req, base) {
  // e.g. req.url="/api/auth/create?x=1", base="/api/auth"
